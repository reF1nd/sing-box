...
func TestCgroupBackendProgramLoadIntegration(t *testing.T) {
	...
	for _, hijackDNS := range []bool{true, false} {
		...
		for _, selfTGID := range []uint32{uint32(os.Getpid()), 0} {
			...
			t.Run(dnsMode+"/"+selfBypass, func(t *testing.T) {
				testCgroupBackendProgramLoad(t, hijackDNS, selfTGID, selfBypass)
			})
		}
	}
}

func testCgroupBackendProgramLoad(t *testing.T, hijackDNS bool, selfTGID uint32, expectedSelfBypass string) {
	...
	if err = backend.loadPrograms(65532, selfTGID); err != nil {
		t.Fatal(err)
	}
	actualSelfBypass := backend.SelfBypassMode()
	if expectedSelfBypass == "socket_cookie" && actualSelfBypass != expectedSelfBypass {
		t.Fatalf("unexpected self bypass mode: %s", actualSelfBypass)
	}
	if expectedSelfBypass == "tgid" && actualSelfBypass == "socket_cookie" {
		t.Log("kernel rejected TGID self bypass; socket-cookie fallback loaded successfully")
	} else if actualSelfBypass != expectedSelfBypass {
		t.Fatalf("unexpected self bypass mode: %s", actualSelfBypass)
	}
...
func TestCgroupBackendTrafficIntegration(t *testing.T) {
	...
	var cachedToken listenerLookupKey
	if flowCacheEnabled {
		if err = lookupMap(backend.udpFlowMapFD, unsafe.Pointer(&flowKey), unsafe.Pointer(&cachedToken)); err != nil {
			t.Fatal("lookup unconnected UDP flow cache: ", err)
		}
		if cachedToken != redirectKey {
			t.Fatalf("unexpected cached UDP token: %+v", cachedToken)
		}
	}
	...
	if flowCacheEnabled {
		if err = lookupMap(backend.udpFlowMapFD, unsafe.Pointer(&flowKey), unsafe.Pointer(&cachedToken)); !errors.Is(err, unix.ENOENT) {
