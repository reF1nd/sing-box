...
the `ebpf_integration` build tag. The program-load test
creates the maps and asks the kernel verifier to load every dual-stack TCP/UDP
program, including the socket-release program, then closes everything without
attaching. The traffic test creates a temporary child cgroup and verifies IPv4
