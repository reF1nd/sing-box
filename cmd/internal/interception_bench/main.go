...
func parseOptions() options {
	...
	flag.StringVar(
		...
		"scenario",
		"all",
		"all or a comma-separated list of tcp-short,tcp-upload,tcp-download,udp-pps,udp-unconnected-pps",
	)
	...
	flag.IntVar(&options.payloadSize, "payload-size", 1200, "TCP block and UDP datagram size")
...
func runClient(options options) error {
	...
	if options.payloadSize < 1 || options.payloadSize > 65507 {
		return errors.New("payload-size must be between 1 and 65507")
	}
...
func parseScenarios(value string) ([]string, error) {
	if value == "all" {
		return []string{"tcp-short", "tcp-upload", "tcp-download", "udp-pps", "udp-unconnected-pps"}, nil
	}
	...
	for _, scenario := range strings.Split(value, ",") {
		...
		switch scenario {
		case "tcp-short", "tcp-upload", "tcp-download", "udp-pps", "udp-unconnected-pps":
		default:
...
func measureTCPUpload(options options, deadline time.Time, result *counters) error {
	return runWorkers(options.concurrency, func() {
		...
		frame := make([]byte, 4+options.payloadSize)
		binary.BigEndian.PutUint32(frame[:4], uint32(options.payloadSize))
		for time.Now().Before(deadline) {
...
func measureConnectedUDP(options options, deadline time.Time, result *counters) error {
	return runWorkers(options.concurrency, func() {
		...
		response := make([]byte, options.payloadSize)
		for time.Now().Before(deadline) {
			_ = connection.SetDeadline(deadline)
			if _, err = connection.Write(payload); err != nil {
