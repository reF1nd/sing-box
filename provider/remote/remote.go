...
func (s *ProviderRemote) loadFromContent(contentRaw []byte) error {
	...
	outboundOpts, err := parser.ParseBoxSubscription(s.ctx, content)
	if err != nil {
		return err
	}
...
func (s *ProviderRemote) updateProviderFromContent(content string) error {
	outboundOpts, err := parser.ParseSubscription(s.ctx, content, s.overrideDialer, s.overrideTLS, s.Tag())
	if err != nil {
		return err
	}
