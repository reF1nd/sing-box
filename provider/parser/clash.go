...
func convertTLSOptions(proxy map[string]any) *option.OutboundTLSOptions {
	...
	if alpn, exists := proxy["alpn"].([]any); exists {
		...
		for _, item := range alpn {
			alpnArr = append(alpnArr, F.ToString(item))
		}
	...
	if wsOpts, exists := proxy["ws-opts"].(map[string]any); exists {
		if path, exists := wsOpts["path"].(string); exists {
			...
			if result[2] != "" {
				...
				options.MaxEarlyData = stringToType[uint32](result[2])
			}
		}
			...
			for key, valueRaw := range headers {
				...
				case []any:
					for _, item := range value {
						valueArr = append(valueArr, F.ToString(item))
					}
				default:
					valueArr = append(valueArr, F.ToString(value))
				}
...
func parseSSClash(proxy map[string]any) (option.Outbound, error) {
	...
	if port, exists := proxy["port"]; exists {
		options.ServerPort = stringToType[uint16](F.ToString(port))
	}
		...
		case "obfs":
			...
			if opts, exists := proxy["plugin-opts"].(map[string]any); exists {
				for key, value := range opts {
					switch key {
					case "mode":
						optArr = append(optArr, F.ToString("obfs=", value))
					case "host":
						optArr = append(optArr, F.ToString("obfs-host=", value))
					default:
						optArr = append(optArr, F.ToString(key, "=", value))
					}
				}
			}
		case "v2ray-plugin":
			...
			if opts, exists := proxy["plugin-opts"].(map[string]any); exists {
				for key, value := range opts {
					...
					case "host":
						...
						optArr = append(optArr, F.ToString("host=", host))
					case "path":
						path := value
						if p, ok := proxy["ws-path"].(string); ok {
							path = p
						}
						optArr = append(optArr, F.ToString("path=", path))
					case "headers":
						headers, _ := value.(map[string]any)
						data, _ := json.Marshal(headers)
						optArr = append(optArr, F.ToString("headers", "=", string(data)))
					case "mux":
...
func parseSTLSClash(proxy map[string]any) (option.Outbound, option.Outbound, error) {
	...
	if port, exists := proxy["port"]; exists {
		stOptions.ServerPort = stringToType[uint16](F.ToString(port))
	}
	if method, exists := proxy["cipher"].(string); exists {
...
func parseTUICClash(proxy map[string]any) (option.Outbound, error) {
	...
	if heartbeat, exists := proxy["heartbeat-interval"].(int); exists {
		options.Heartbeat = stringToType[badoption.Duration](F.ToString(heartbeat, "ms"))
	}
	options.TLS = convertTLSOptions(proxy)
...
func parseHysteriaClash(proxy map[string]any) (option.Outbound, error) {
	...
	if authStr, exists := proxy["auth-str"]; exists {
		options.AuthString = F.ToString(authStr)
	}
	...
	if ca, exists := proxy["ca"]; exists {
		options.TLS.CertificatePath = F.ToString(ca)
	}
	if caStr, exists := proxy["ca-str"].([]any); exists {
		var caStrArr []string
		for _, item := range caStr {
			caStrArr = append(caStrArr, F.ToString(item))
		}
