...
func TestAnyTLSSelf(t *testing.T) {
	const idleCheckInterval = 6 * time.Second
	// The pinned AnyTLS library checks cancellation only after its cleanup
	// sleep. Wait after all instances close so TestMain can check for leaks.
	t.Cleanup(func() { time.Sleep(idleCheckInterval + time.Second) })
				...
				startInstance(t, option.Options{
					...
					Outbounds: []option.Outbound{
						...
						{Type: C.TypeAnyTLS, Tag: "anytls-out", Options: &option.AnyTLSOutboundOptions{
							...
							IdleSessionCheckInterval: badoption.Duration(idleCheckInterval),
						}},
					},
