...
func (t *Transport) updateTransports(link *TransportLink) error {
	...
	for _, address := range link.address {
		...
		if link.dnsOverTLS {
			...
			transports = append(transports, transport.NewTLSRaw(t.ctx, t.logger, t.TransportAdapter, serverDialer, M.SocksaddrFrom(serverAddr, 53), tlsConfig, false, C.TCPKeepAliveInterval, 0))
		} else {
	...
	for _, address := range link.addressEx {
		...
		if link.dnsOverTLS {
			...
			transports = append(transports, transport.NewTLSRaw(t.ctx, t.logger, t.TransportAdapter, serverDialer, M.SocksaddrFrom(serverAddr, address.Port), tlsConfig, false, C.TCPKeepAliveInterval, 0))
		} else {
