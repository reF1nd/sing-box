...
| 自身防回环 | 新内核优先在 socket-cookie map 前比较 TGID；任一 attach type 不支持时自动回退到纯 cookie 程序 | 使用 dae 自己的 cgroup/socket metadata 与 netns 数据路径 |
...
| 新内核优化 | verifier 接受时使用 TGID 自身绕过，同时始终保留 socket-cookie 自动回退 | 6.7+ 可尝试 netkit；满足安全条件的 6.8+ 可使用 `bpf_redirect_peer`，否则回退 veth/普通 redirect |
