...
| Self-bypass | Modern kernels use a TGID comparison before the socket-cookie map; unsupported attach types automatically fall back to cookie-only programs | Uses dae's own cgroup/socket metadata and namespace data path |
...
| BTF/CO-RE | Does not require kernel BTF; cgroup programs are generated as raw instructions and the TC object does not use CO-RE | Requires BTF, CO-RE, and a broader eBPF/kprobe feature set |
...
| New-kernel optimization | Uses TGID self-bypass when accepted by the verifier and automatically retains socket-cookie fallback | Linux 6.7+ may use netkit; suitable Linux 6.8+ setups may use `bpf_redirect_peer`, otherwise dae falls back to veth or normal redirect |
...
transparent layer-2 bridge proxy. sing-box uses TC priority `1` and bypassed
