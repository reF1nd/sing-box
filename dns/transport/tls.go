...
func NewTLS(ctx context.Context, logger log.ContextLogger, tag string, options option.RemoteTLSDNSServerOptions) (adapter.DNSTransport, error) {
	...
	idleTimeout := time.Duration(options.TCPKeepAliveInterval)
	if idleTimeout == 0 {
		idleTimeout = C.TCPKeepAliveInterval
	}
	...
	return NewTLSRaw(ctx, logger, dns.NewTransportAdapterWithRemoteOptions(C.DNSTypeTLS, tag, options.RemoteDNSServerOptions), transportDialer, serverAddr, tlsConfig, options.Pipeline, idleTimeout, maxQueries), nil
}

func NewTLSRaw(ctx context.Context, logger logger.ContextLogger, adapter dns.TransportAdapter, dialer N.Dialer, serverAddr M.Socksaddr, tlsConfig tls.Config, enablePipeline bool, idleTimeout time.Duration, maxQueries int) *TLSTransport {
...
func (t *TLSTransport) createNewConnection(ctx context.Context, message *mDNS.Msg) (*mDNS.Msg, error) {
	...
	conn := newReuseableDNSConn(tlsConn, t.logger, t.enablePipeline, t.idleTimeout, t.maxQueries, t.connections, t)
